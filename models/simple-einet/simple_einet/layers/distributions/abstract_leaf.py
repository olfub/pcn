...
class AbstractLeaf(AbstractLayer, ABC):
    ...
    def _marginalize_scaled(self, x: torch.Tensor, marginalized_scopes: List[int], d: dist.Distribution) -> torch.Tensor:
        """
        Marginalizes the input tensor `x` along the dimensions specified in `marginalized_scopes`.
        In this setting, probabilities can be scaled such that they do not sum to 1.0.
        This has to be accounted for during marginalization.

        Args:
            x (torch.Tensor): The input tensor to be marginalized.
            marginalized_scopes (List[int]): A list of dimensions to be marginalized.
            d (dist.Distribution): The (scaled) probability distributions in the leaf nodes.
        ...
        if marginalized_scopes is not None:
            ...
            probs_sum = torch.log(d.probs[:, :, s, :, :, :].sum(dim=-1))
            x[:, :, s] = probs_sum
        else:
            if torch.any(mask := torch.isnan(x)):
                x[mask] = self.marginalization_constant
    ...
    def forward(self, x, marginalized_scopes: List[int], scaling_factors: torch.Tensor = None):
        """
        ...
        if scaling_factors is not None:
            # TODO make scaling factors work for other leaves
            from simple_einet.layers.distributions.categorical import Categorical
            assert (type(self) == Categorical), \
                f"Scaling factors can currently only be applied to Categorical leaf distributions but got {self.config.leaf_type}."
            assert (scaling_factors.shape[0] == d.probs.shape[2]) and (scaling_factors.shape[1] == d.probs.shape[5])
            
            # Scale the probabilities and logits of the distribution by the scaling factors
            safe_scaling = scaling_factors.clamp(min=1e-10)  # avoid log(0)
            d.probs *= scaling_factors[None, None, :, None, None, :]
            d.logits += torch.log(safe_scaling)[None, None, :, None, None, :]
        ...
        if scaling_factors is not None:
            x = self._marginalize_scaled(x, marginalized_scopes, d)
        else:
