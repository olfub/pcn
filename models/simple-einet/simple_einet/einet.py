...
class Einet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, marginalized_scopes: torch.Tensor = None, cache_index: Optional[int] = None, scaling_factors: torch.Tensor = None) -> torch.Tensor:
        """
        ...
        Args:
            ...
            scaling_factors (torch.Tensor): Scaling factors for the leaf distributions. If not None, these are applied to the leaf distributions.
        ...
        if cache_index is not None and cache_index in self._leaf_cache:
            ...
            x = self.leaf(x, marginalized_scopes, scaling_factors=scaling_factors)
